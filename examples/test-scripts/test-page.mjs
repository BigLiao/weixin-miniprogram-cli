...
async function main() {
  ...
  await describe('get_current_page', async () => {
    const result = await run('get_current_page', {});
    assert(result.includes('当前页面'), '应返回当前页面信息');
    assert(ctx.currentPage !== null, 'currentPage 应已设置');
  });

  await describe('get_page_snapshot', async () => {
    const result = await run('get_page_snapshot', { format: 'compact' });
    assert(result.includes('快照获取成功'), '应获取快照成功');
    assert(ctx.elementMap.size > 0, '应有元素映射');
    console.log(`    元素数量: ${ctx.elementMap.size}`);
  });

  await describe('query_selector', async () => {
    const result = await run('query_selector', { selector: 'view' });
    assert(result.includes('找到元素'), '应找到 view 元素');
  });

  await describe('wait_for (delay)', async () => {
    const start = Date.now();
    const result = await run('wait_for', { delay: 500 });
    const elapsed = Date.now() - start;
    assert(result.includes('等待') && result.includes('完成'), '延时等待应完成');
    assert(elapsed >= 450, '应至少等待 ~500ms');
  });

  await describe('wait_for (selector)', async () => {
    const result = await run('wait_for', { selector: 'view', timeout: 3000 });

...
async function main() {
  ...
  await describe('navigate_to', async () => {
    const result = await run('navigate_to', { url: 'page/component/index' });
    assert(result.includes('导航到'), '应导航成功');
  });

  await describe('navigate_back', async () => {
    const result = await run('navigate_back', { delta: 1 });
    assert(result.includes('返回到'), '应返回成功');
  });

  await describe('switch_tab', async () => {
    // miniprogram-demo 的 tab 页面（根据实际配置调整）
    try {
      const result = await run('switch_tab', { url: 'page/component/index' });
      assert(result.includes('切换到') || result.includes('失败'), 'switch_tab 应执行');
    } catch (e) {
      // 如果不是 tab 页面会失败，这是预期的
      assert(true, `switch_tab: ${e.message}`);
    }
  });

...
export class SessionManager {
  ...
  resolve(explicitId?: string): Session {
    ...
    if (explicitId) {
      ...
      if (!s) {
        throw new Error(`Session "${explicitId}" 不存在。使用 session list 查看所有 session`);
      }
      return s;
    }
    ...
    // 4. 没有 session
    if (all.length === 0) {
      throw new Error('没有可用的 session。请先执行 open 或 launch');
    }
    ...
    throw new Error(
      ...
      '请使用 --session <id> 指定，或执行 session use --id <id> 切换默认 session'
    );
  }
